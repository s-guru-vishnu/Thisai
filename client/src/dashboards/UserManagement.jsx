...
const UserManagement = () => {
    ...
    const handleRecruit = async (driverId) => {
        if (!window.confirm(`Add this driver to your operational team?`)) return;

        try {
            setUpdating(driverId);
            const apiBase = import.meta.env.VITE_API_BASE_URL || 'http://localhost:5005';
            await axios.put(`${apiBase}/api/manager/drivers/${driverId}/recruit`, {}, {
                headers: { Authorization: `Bearer ${userInfo.token}` }
            });
            
            // Update local state
            setUsers(users.map(u => u._id === driverId ? { ...u, assignedManager: userInfo._id } : u));
            setError(null);
            alert("Driver successfully recruited to your team!");
        } catch (err) {
            setError(err.response?.data?.message || 'Failed to recruit driver');
        } finally {
            setUpdating(null);
        }
    };
                                            ...
                                            <td style={{ padding: '1rem', textAlign: 'right' }}>
                                                {!['driver', 'cargo_driver', 'delivery_driver'].includes(user.role) ? (
                                                    ...
                                                    <div style={{ display: 'flex', gap: '10px', alignItems: 'center', justifyContent: 'flex-end' }}>
                                                        ...
                                                        {userInfo.role === 'manager' && user.assignedManager !== userInfo._id && (
                                                            <button 
